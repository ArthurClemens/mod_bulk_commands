...
var action_bulk_commands = (function ($) {
  ...
  var controlsTmpl = [
    ...
    '            <a class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="#">Select <b class="caret"></b></a>',
  ...
  var controlsTmplDelete = [
    ...
    '            <a class="btn btn-default adminBulkCommandsDelete" href="#">Delete</a>',
    "        </div>",
  ].join("\n");

  var controlsTmplAttributes = [
    '        <div class="btn-group dropdown adminBulkCommand">',
    '            <a class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="#">Set state... <b class="caret"></b></a>',
    '            <ul class="dropdown-menu" role="menu">',
  ...
  var deletePopoverControlsTmpl = [
    '<a href="#xxx" class="btn btn-danger" data-action="delete">Delete</a>',
    '<a href="#" class="btn btn-default" data-action="cancel">Cancel</a>',
  ].join("\n");

  var checkboxTmpl =
    '<td class=\'not-clickable\'><input type="checkbox" class="adminBulkCommandCheck" /></td>';
  ...
  var createControlsTemplate = function () {
    ...
    if (
      ...
      attributeListElems = attributeList.join(controlsTmplAttributesSeparator);
      attributes = controlsTmplAttributes.replace(
        "PLACEHOLDER_ATTRIBUTES",
        attributeListElems
      );
    }
    controlsTemplate = controlsTemplate.replace(
      "PLACEHOLDER_ATTRIBUTES_DROPDOWN",
      attributes
    );
  ...
  var tableController = function ($table) {
    ...
    setUpControlsUI = function () {
      ...
      $(".adminBulkCommandsDelete", $controls)
        .popover({
    ...
    sendCommand = function (command) {
      console.log("getCheckedIds()", command, getCheckedIds());
  ...
  $.ui.adminLinkedTable.prototype._originalInit =
    $.ui.adminLinkedTable.prototype._init;
  $.widget(
    "ui.adminLinkedTable",
    $.extend({}, $.ui.adminLinkedTable.prototype, {
      _init: function () {
        var $table, controller, initResult;
        initResult = $.ui.adminLinkedTable.prototype._originalInit.apply(
          this,
          arguments
        );
      ...
      setTimeout(function () {
        ...
        if (o.command === "delete") {
          ...
          if ($pagination.length > 0) {
            ...
            var validHrefs = $pagination.find("li a").filter(function () {
